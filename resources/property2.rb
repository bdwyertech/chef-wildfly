...
resource_name :wildfly_property2

# => Define the Resource Properties
property :property, String, required: true
...
action_class.class_eval do
  ...
  def jb_cli(cmd)
    cmd = Mixlib::ShellOut.new("bin/jboss-cli.sh -c '#{cmd}'")
    cmd.user = node['wildfly']['user']
    cmd.cwd  = node['wildfly']['base']
  ...
  def convert_to_hash(hash_text)
    ...
    JSON.parse(hash_text) rescue {} # rubocop: disable RescueModifier
  end
end
